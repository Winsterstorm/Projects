					...
					while (wasIt){
						if (wasIt) {wasIt=false;}
							...
							ball[i].m=Math.trunc(Math.PI*ball[i].r**2*c);
								...
								for (var k=0; k<i; k++)
					if ((ball[k].x-ball[i].x)**2 + (ball[k].y-ball[i].y)**2 < (ball[i].r+ball[k].r)**2) {
						wasIt = true;}
...
function phys() { // теперь будем работать с этими значениями
  ...
  function dynamics() {
    	...
    	ball[i]["vx"] = v2x*(2*m2)/(m1 + m2);// зси + зсэ 
    	ball[i]["vy"] = v2y*(2*m2)/(m1 + m2);
    	ball[j]["vx"] = v1x*(2*m1)/(m1 + m2);
    	ball[j]["vy"] = v1y*(2*m1)/(m1 + m2);
    }
